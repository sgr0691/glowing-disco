...
import { Kind, isScalarType, isEqualType, isListType, isNonNullType, isObjectType, isAbstractType, isEnumType, isInterfaceType, isUnionType, GraphQLObjectType, visit, print as print$1, SchemaMetaFieldDef, TypeMetaFieldDef, parse } from 'graphql';
import { resolveExternalModuleAndFn, ApolloFederation, getBaseType, toPascalCase as toPascalCase$1 } from '@graphql-codegen/plugin-helpers';
...
import { Parser } from 'relay-compiler';
import { transform as transform$1 } from 'relay-compiler/lib/transforms/InlineFragmentsTransform';
import { transform as transform$2 } from 'relay-compiler/lib/transforms/SkipRedundantNodesTransform';
import { transform } from 'relay-compiler/lib/transforms/ApplyFragmentArgumentTransform';
...
class ClientSideBaseVisitor extends BaseVisitor {
    ...
    get fragmentsGraph() {
        ...
        for (const fragment of this._fragments) {
            if (graph.hasNode(fragment.name)) {
                const cachedAsString = print$1(graph.getNodeData(fragment.name).node);
                const asString = print$1(fragment.node);
    ...
    OperationDefinition(node) {
        ...
        const operationType = toPascalCase$1(node.operation);
...
function optimizeOperations(schema, documents) {
    ...
    const relayDocuments = Parser.transform(adjustedSchema, documentAsts);
    const queryCompilerContext = new GraphQLCompilerContext(adjustedSchema)
        .addAll(relayDocuments)
        .applyTransforms([transform, transform$1, transformWithOptions({ flattenAbstractTypes: false }), transform$2]);
