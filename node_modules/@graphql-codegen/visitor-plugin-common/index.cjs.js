...
function toPascalCase(str, transformUnderscore = false) {
    return convertNameParts(str, changeCase.pascalCase, transformUnderscore);
}
...
function convertFactory(config) {
    function resolveConventionName(type) {
        if (!config.namingConvention) {
            return (str, opts = {}) => {
                return convertNameParts(str, changeCase.pascalCase, getConfigValue((opts || {}).transformUnderscore, false));
            };
        }
        ...
        if (typeof config.namingConvention === 'object') {
            if (!config.namingConvention[type]) {
                return (str, opts = {}) => {
                    ...
                    return convertNameParts(str, changeCase.pascalCase, getConfigValue(transformUnderscore, false));
                };
            }
            return (str, opts = {}) => {
...
function parseEnumValues(schema, mapOrStr) {
    ...
    if (typeof mapOrStr === 'object') {
        ...
        if (invalidMappings.length > 0) {
            throw new core.DetailedError(`Invalid 'enumValues' mapping!`, `The following types does not exist in your GraphQL schema: ${invalidMappings.join(', ')}`);
        }
        return Object.keys(mapOrStr).reduce((prev, gqlIdentifier) => {
            ...
            if (typeof pointer === 'string') {
                ...
                throw new core.DetailedError(`Invalid "enumValues" configuration`, `Enum "${gqlIdentifier}": expected string or object (with enum values mapping)`);
            }
        }, {});
    }
...
class ClientSideBaseVisitor extends BaseVisitor {
        ...
        if (fragments && fragments.length > 0) {
            if (this.config.documentMode === exports.DocumentMode.documentNode) {
                return `${fragments
                    ...
                    if (found) {
                        return graphql.print(found.node);
                    }
                    return null;
                })
                    .filter(a => a)
    ...
    _generateFragment(fragmentDocument) {
        ...
        return `export const ${name}${this.config.documentMode === exports.DocumentMode.documentNode ? ': DocumentNode' : ''} = ${this._gql(fragmentDocument)};`;
    }
    get fragmentsGraph() {
    ...
    getImports() {
            ...
            case exports.DocumentMode.external:
                if (this._collectedOperations.length > 0) {
                    if (this.config.importDocumentNodeExternallyFrom === 'near-operation-file' && this._documents.length === 1) {
                        imports.push(`import * as Operations from './${this.clearExtension(path.basename(this._documents[0].filePath))}';`);
                    }
                    else {
        ...
        if (this.config.documentMode === exports.DocumentMode.graphQLTag) {
            (this._fragments || [])
    ...
    OperationDefinition(node) {
        ...
        if (this.config.documentMode !== exports.DocumentMode.external) {
            documentString = `${this.config.noExport ? '' : 'export'} const ${documentVariableName}${this.config.documentMode === exports.DocumentMode.documentNode ? ': DocumentNode' : ''} = ${this._gql(node)};`;
        }
        const operationType = pluginHelpers.toPascalCase(node.operation);
...
function optimizeOperations(schema, documents) {
    ...
    const adjustedSchema = RelayCreate.create(common.printSchemaWithDirectives(schema));
    const relayDocuments = relayCompiler.Parser.transform(adjustedSchema, documentAsts);
    ...
    const newQueryDocuments = queryCompilerContext.documents().map(doc => ({
        ...
        content: graphql.parse(print(adjustedSchema, doc)),
    }));
