        ...
        if (err) {
            ...
            if (errorCount > 0) {
                ...
                const details = err.errors
                    ...
                    return { msg: core.isDetailedError(error) ? error.details : null, rawError: error };
                })
                    .map(({ msg, rawError }, i) => {
...
async function getPluginByName(name, pluginLoader) {
    ...
    for (const moduleName of possibleModules) {
        try {
            ...
            if (err.message.indexOf(`Cannot find module '${moduleName}'`) === -1) {
                throw new core.DetailedError(`Unable to load template plugin matching ${name}`, `
    ...
    `.trimRight())
        .join('');
    throw new core.DetailedError(`Unable to find template plugin matching ${name}`, `
...
async function getPresetByName(name, loader) {
    ...
    for (const moduleName of possibleModules) {
        try {
            ...
            if (err.message.indexOf(`Cannot find module '${moduleName}'`) === -1) {
                throw new core.DetailedError(`Unable to load preset matching ${name}`, `
    ...
    `.trimRight())
        .join('');
    throw new core.DetailedError(`Unable to find preset matching ${name}`, `
...
function tryToBuildSchema(schema) {
    try {
        return graphql.buildASTSchema(schema, { commentDescriptions: true });
    }
    catch (e) {
...
const loadSchema = async (schemaPointers, config) => {
    try {
        const docs = await core$1.loadTypedefsUsingLoaders([new codeFileLoader.CodeFileLoader(), new gitLoader.GitLoader(), new githubLoader.GithubLoader(), new graphqlFileLoader.GraphQLFileLoader(), new jsonFileLoader.JsonFileLoader(), new urlLoader.UrlLoader()], schemaPointers, config);
        return schemaMerging.mergeTypeDefs(docs.map(({ document }) => document));
    }
    catch (e) {
        throw new core.DetailedError('Failed to load schema', `
...
const loadDocuments = async (documentPointers, config) => {
    const loadedFromToolkit = await core$1.loadDocumentsUsingLoaders([new codeFileLoader.CodeFileLoader(), new gitLoader.GitLoader(), new githubLoader.GithubLoader(), new graphqlFileLoader.GraphQLFileLoader()], documentPointers, {
...
async function loadContext(configFilePath) {
    ...
    if (!result) {
        if (configFilePath) {
            throw new core.DetailedError(`Config ${configFilePath} does not exist`, `
    ...
    if (result.isEmpty) {
        throw new core.DetailedError(`Found Codegen config file but it was empty!`, `
...
class CodegenContext {
    ...
    loadSchema(pointer) {
        if (this._graphqlConfig) {
            ...
            return this._graphqlConfig.getProject(this._project).loadSchema(pointer, 'DocumentNode');
        }
...
async function executeCodegen(input) {
    ...
    async function normalize() {
        ...
        if (generateKeys.length === 0) {
            throw new core.DetailedError('Invalid Codegen Configuration!', `
