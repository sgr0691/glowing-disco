...
function mergeSchemas(schemas) {
    ...
    if (compactSchemas.length === 0) {
        ...
        return schemaMerging.mergeTypeDefs(compactSchemas);
    }
}
...
async function executePlugin(options, plugin) {
    ...
    const outputSchema = schema || graphql.buildASTSchema(options.schema);
...
async function codegen(options) {
    ...
    if (isFederation) {
        schemaChanged = true;
        schema = pluginHelpers.turnExtensionsIntoObjectTypes(mergeSchemas([schema, pluginHelpers.federationSpec]));
    }
    if (schemaChanged) {
        options.schemaAst = graphql.buildASTSchema(schema, {
    ...
    if (options.schemaAst && documents.length > 0 && !skipDocumentValidation) {
        ...
        const errors = await common.validateGraphQlDocuments(options.schemaAst, [
            ...documents.map(({ filePath, content }) => ({ location: filePath, document: content })),
            ...extraFragments.map((f) => ({ location: f.importFrom, document: { kind: graphql.Kind.DOCUMENT, definitions: [f.node] } })),
        ]);
