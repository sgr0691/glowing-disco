...
class TsVisitor extends visitorPluginCommon.BaseTypesVisitor {
    ...
    FieldDefinition(node, key, parent) {
        ...
        return comment + visitorPluginCommon.indent(`${this.config.immutableTypes ? 'readonly ' : ''}${node.name}${addOptionalSign ? '?' : ''}: ${typeString},`);
    }
    InputValueDefinition(node, key, parent) {
        ...
        const comment = visitorPluginCommon.transformComment(node.description, 1);
        return comment + visitorPluginCommon.indent(`${this.config.immutableTypes ? 'readonly ' : ''}${node.name}${addOptionalSign ? '?' : ''}: ${node.type},`);
    }
...
function includeIntrospectionDefinitions(schema, documents, config) {
    ...
    documents.forEach(doc => graphql.visit(doc.content, documentsVisitor));
