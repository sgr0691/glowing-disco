...
class ReactApolloVisitor extends visitorPluginCommon.ClientSideBaseVisitor {
    ...
    _buildHocProps(operationName, operationType) {
        const typeVariableName = this._externalImportPrefix + this.convertName(operationName + pluginHelpers.toPascalCase(operationType) + this._parsedConfig.operationResultSuffix);
        const variablesVarName = this._externalImportPrefix + this.convertName(operationName + pluginHelpers.toPascalCase(operationType) + 'Variables');
  ...
  ${propsTypeName}<TChildProps>>) {
    return ApolloReactHoc.with${changeCase.pascalCase(node.operation)}<TProps, ${operationResultType}, ${operationVariablesTypes}, ${propsTypeName}<TChildProps>>(${this.getDocumentNodeVariable(node, documentVariableName)}, {
      alias: '${changeCase.camelCase(operationName)}',
      ...operationOptions
    });
};`;
 ...
 * const [${changeCase.camelCase(operationName)}, { data, loading, error }] = use${operationName}({
 *   variables: {${variableString}
 *   },
 * });`;
        return `
 ...
 */`;
    ...
    _buildHooks(node, operationType, documentVariableName, operationResultType, operationVariablesTypes) {
        ...
        if (operationType === 'Query') {
            const lazyOperationName = this.convertName(node.name.value, {
                suffix: changeCase.pascalCase('LazyQuery'),
                useTypesPrefix: false,
            });
    ...
    _getHookSuffix(name, operationType) {
        if (!this.config.dedupeOperationSuffix) {
            return changeCase.pascalCase(operationType);
        }
