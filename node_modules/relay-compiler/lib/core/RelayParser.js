...
function () {
  ...
  _proto.transform = function transform() {
    ...
    eachWithCombinedError(this._definitions, function (_ref4) {
      var name = _ref4[0],
          definition = _ref4[1];
    ...
    eachWithCombinedError(entries.values(), function (_ref5) {
  ...
  if (schema.isList(nullableType)) {
    ...
    ast.values.forEach(function (item) {
      ...
      if (itemValue.kind === 'Literal') {
        ...
        throw createUserError('Complex argument values (Lists or InputObjects with nested variables) are not supported.', null, [item]);
      }
    });
    return {
      kind: 'Literal',
    ...
    ast.fields.forEach(function (field) {
      ...
      if (fieldValue.kind === 'Literal') {
        ...
        throw createUserError('Complex argument values (Lists or InputObjects with nested variables) are not supported.', null, [field.value]);
      }
    });
    return {
      kind: 'Literal',
...
function checkFragmentSpreadTypeCompatibility(schema, fragmentType, parentType, fragmentName, fragmentTypeAST, parentTypeAST) {
  if (!schema.doTypesOverlap(fragmentType, schema.assertCompositeType(parentType))) {
    ...
    if (possibleConcreteTypes.length !== 0) {
      suggestedTypesMessage = " Possible concrete types include ".concat(possibleConcreteTypes.slice(0, 3).map(function (type) {
