...
function _writeRelayGeneratedFile() {
  _writeRelayGeneratedFile = _asyncToGenerator(function* (schema, codegenDir, definition, _generatedNode, formatModule, typeText, _persistQuery, platform, sourceHash, extension) {
    ...
    var writeQueryParameters = arguments.length > 12 ? arguments[12] : undefined;
    ...
    if (generatedNode.kind === RelayConcreteNode.REQUEST) {
      ...
      var oldHash = Profiler.run('RelayFileWriter:compareHash', function () {
        oldContent = codegenDir.read(filename); // Hash the concrete node including the query text.
      ...
      var oldRequestParameters = extractRelayRequestParams(oldContent);

      if (!shouldRepersist && hash === oldHash) {
        codegenDir.markUnchanged(filename);

        if (writeQueryParameters && oldRequestParameters && queryParametersFilename != null && generatedNode.kind === RelayConcreteNode.REQUEST && generatedNode.params.operationKind === 'query') {
          writeQueryParameters(codegenDir, queryParametersFilename, moduleName, oldRequestParameters);
        }

        return oldRequestParameters ? (0, _objectSpread2["default"])({}, generatedNode, {
...
function extractRelayRequestParams(text) {
  if (text == null || text.length === 0) {
    return null;
  }
  ...
  var match = text.match(/@relayRequestParams (.+)/);
  var requestParams;

  try {
    var _ref;

    requestParams = JSON.parse((_ref = match === null || match === void 0 ? void 0 : match[1]) !== null && _ref !== void 0 ? _ref : '');
  } catch (_unused) {}
