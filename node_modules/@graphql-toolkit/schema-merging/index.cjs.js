...
function fixSchemaAst(schema, config) {
    return graphql.buildASTSchema(graphql.parse(graphql.printSchema(schema, { commentDescriptions: config.commentDescriptions })));
}
...
function mergeSchemas({ schemas, typeDefs, resolvers, resolversComposition, schemaDirectives, resolverValidationOptions, logger, exclusions }) {
    return graphqlTools.makeExecutableSchema({
        typeDefs: mergeTypeDefs([...schemas, ...(typeDefs ? common.asArray(typeDefs) : [])], { exclusions }),
        resolvers: common.composeResolvers(mergeResolvers([...schemas.map(schema => common.extractResolversFromSchema(schema)), ...(resolvers ? common.asArray(resolvers) : [])], { exclusions }), resolversComposition || {}),
        schemaDirectives,
...
async function mergeSchemasAsync({ schemas, typeDefs, resolvers, resolversComposition, schemaDirectives, resolverValidationOptions, logger, exclusions }) {
    const [typeDefsOutput, resolversOutput] = await Promise.all([
        mergeTypeDefs([...schemas, ...(typeDefs ? common.asArray(typeDefs) : [])], { exclusions }),
        Promise.all(schemas.map(async (schema) => common.extractResolversFromSchema(schema))).then(extractedResolvers => common.composeResolvers(mergeResolvers([...extractedResolvers, ...(resolvers ? common.asArray(resolvers) : [])], { exclusions }), resolversComposition || {})),
    ]);
    return graphqlTools.makeExecutableSchema({
